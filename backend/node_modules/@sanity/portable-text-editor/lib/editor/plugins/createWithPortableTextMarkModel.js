          ...
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (op.type === 'merge_node' && op.path.length === 1 && 'markDefs' in op.properties && op.properties._type === portableTextFeatures.types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
              ...
              if (editor.isTextBlock(targetBlock)) {
                ...
                var newMarkDefs = (0, _uniq2.default)([...oldDefs, ...op.properties.markDefs]);
                var isNormalized = (0, _isEqual2.default)(newMarkDefs, targetBlock.markDefs); // eslint-disable-next-line max-depth
            ...
            if (op.type === 'split_node' && op.path.length === 1 && op.properties._type === portableTextFeatures.types.block.name && 'markDefs' in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0) {
              var _Editor$node7 = _slate.Editor.node(editor, [op.path[0]]),
                  _Editor$node8 = _slicedToArray(_Editor$node7, 2),
                  block = _Editor$node8[0],
                  blockPath = _Editor$node8[1];

              if (editor.isTextBlock(block) && block.children.length === 1 && block.markDefs.length > 0 && _slate.Text.isText(block.children[0]) && block.children[0].text === '' && block.children[0].marks.length === 0) {
                _slate.Transforms.setNodes(editor, {
                  markDefs: []
                }, {
                  at: blockPath
